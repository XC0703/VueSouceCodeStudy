...
export function createTransformContext(
	root,
	{ hoistStatic = false, cacheHandlers = false, nodeTransforms = [], directiveTransforms = {} }
) {
	const context = {
		// options
...
// 创建处理节点的上下文
export function createTransformContext(root, { nodeTransforms = [], directiveTransforms = {} }) {
	const context = {
		// plugin
		nodeTransforms,
		directiveTransforms,
...
接着就是来实现真正的核心功能了，可以看到上面整个 `transform` 函数写完都没有出现过 `codegenNode` 的创建，实际上是在 `transformElement` 中生成的，这只是其中一个转换插件，但这个比较重要，就优先实现它。
...
```vue
<template>
	<ChildComponent v-bind="{ prop1: 'value1', prop2: 'value2' }" additionalProp="extraValue" />
</template>
```
...
if ((patchFlag === 0 || patchFlag === PatchFlags.NEED_HYDRATION) && runtimeDirectives.length > 0) {
...
if (propsExpression) {
	switch (propsExpression.type) {
		...
		case NodeTypes.JS_OBJECT_EXPRESSION:
			...
			if (!hasDynamicKey) {
				...
				if (
					...
					(hasStyleBinding || styleProp.value.type === NodeTypes.JS_ARRAY_EXPRESSION)
				) {
		...
		default:
			propsExpression = createCallExpression([createCallExpression([propsExpression])]);
			break;
	}
}
```
...
export const transformOn = dir => {
	...
	if (arg.type === NodeTypes.SIMPLE_EXPRESSION) {
		if (arg.isStatic) {
			...
			eventName = createSimpleExpression(toHandlerKey(capitalize(rawName)), true);
		}
	...
	const ret = {
		props: [createObjectProperty(eventName, exp || createSimpleExpression('() => {}', false))]
	};
	return ret;
};
```
